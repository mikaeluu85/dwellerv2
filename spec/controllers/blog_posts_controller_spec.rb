...
RSpec.describe BlogPostsController, type: :controller do
  ...
  describe 'GET #show' do
    context 'when the blog post is visible' do
      it 'returns a successful response' do
        get :show, params: { category_slug: category.slug, slug: visible_blog_post.slug }
        expect(response).to be_successful
      end

      it 'assigns the correct blog post to @blog_post' do
        get :show, params: { category_slug: category.slug, slug: visible_blog_post.slug }
    ...
    context 'when the blog post is not visible' do
      it 'redirects to the blog overview path' do
        get :show, params: { category_slug: category.slug, slug: hidden_blog_post.slug }
  ...
  describe 'GET #category' do
    ...
    it 'assigns visible blog posts for the category to @blog_posts' do
      visible_post = create(:blog_post, visible: true, category: category)
      hidden_post = create(:blog_post, visible: false, category: category)
      get :category, params: { category_slug: category.slug }
      expect(assigns(:blog_posts)).to include(visible_post)
      expect(assigns(:blog_posts)).not_to include(hidden_post)
    end
  end
